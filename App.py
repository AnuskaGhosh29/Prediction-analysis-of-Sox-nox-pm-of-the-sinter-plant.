...
for tab, key in zip(tabs, PAIRS):
    with tab:
        ...
        st.markdown(
            ...
            + stat("R² gap", f"{mdl['gap']:.4f}", f"test R² {mdl['test_r2']:.4f}",
                   EMBER if mdl["gap"] > 0.3 else INK)
            + stat(f"Within ±{tol:g}", f"{(~off).mean() * 100:.0f}%",
                   f"{int((~off).sum())} of {len(df)} points",
                   MOSS if (~off).mean() >= 0.8 else EMBER)
            + stat("Largest miss", f"{df.loc[worst, 'Error']:+.3f}", f"row {worst}", EMBER)
